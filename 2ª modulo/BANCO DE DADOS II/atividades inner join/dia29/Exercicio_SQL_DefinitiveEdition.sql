...
select  AVG(preco) from produtos where estoque > 0;
...
select clienteid, count(*), month(dataPedidos)  from pedidos group by month(datapedidos);

-- 10 - Encontrar o cliente com o maior número de pedidos.

select Pedidos,  max(quantidade), from pedidos group by pedidos;
